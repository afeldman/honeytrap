...
impl DeceptionSystem {
    ...
    /// Honeypot deployen
    pub async fn deploy_honeypot(&self, config: HoneypotConfig) -> Result<(), Box<dyn std::error::Error>> {
        ...
        tracing::info!("🍯 Deploying {} honeypot on port {}", config.honeypot_type, config.port);
